...
const Topics = () => {
    ...
    function redirectCourse() {
        ...
        if (type === 'video & text course') {
            ...
            const prompt = `Teach me extensively like a teacher would and also use citations from eductaional websites about this subtopic of ${mainTopic} with examples :- ${firstSubtopic.title}. Please Strictly Don't Give Additional Resources And Images.`;
    ...
    async function sendTranscript(url, subtopic) {
        ...
        try {
            ...
            try {
                ...
                const prompt = `Teach Me like a teacher would  this theory in a teaching way and. :- ${concatenatedText}.`;
                sendSummery(prompt, url);
            } catch (error) {
                const prompt = `Teach Me like a  real life professional Teacher would teach about this subtopic of ${mainTopic} with examples and also the texts should be valid from different educational websites:- ${subtopic}. Please Strictly Don't Give Additional Resources And Images.`;
                sendSummery(prompt, url);
            }

        } catch (error) {
            const prompt = `Teach me about this subtopic of ${mainTopic} with examples :- ${subtopic}. Please Strictly Don't Give Additional Resources And Images.`;
            sendSummery(prompt, url);
        }
    }
            ...
            <div className='dark:bg-black flex-1'>
                <div>
                    <div className='max-md:max-w-sm max-sm:max-w-xs max-w-lg m-auto py-10'>
                        ...
                        <Button onClick={redirectCourse} isProcessing={processing} processingSpinner={<AiOutlineLoading className="h-6 w-6 animate-spin" />} className='items-center justify-center text-center dark:bg-white dark:text-black bg-black text-white font-bold rounded-none w-full enabled:hover:bg-black enabled:focus:bg-black enabled:focus:ring-transparent dark:enabled:hover:bg-white dark:enabled:focus:bg-white dark:enabled:focus:ring-transparent mt-10 mb-2'>Teach Me</Button>
